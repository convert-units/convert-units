import {convertString} from "./definitions/angle";
...
var convert
  ...
  , measures = {
    ...
    , fluxDensity: require('./definitions/fluxDensity')
    }
  , Converter;
...
Converter.prototype.to = function (to) {
  ...
  if (this.origin.unit.format_divisor) {
    this.val = convertString(this.val, this.origin.unit.format_divisor);
  }
  ...
  */
  if (this.destination.unit.format_divisor) {
    result = format(result, this.destination.unit.format_divisor);
    return result;
  }
  ...
  */
  each(this.possibilities(), function(possibility) {
    ...
    if (isIncluded && unit.system === this.origin.system) {
      ...
      if (!best || (result < best.val)) {
        if ((result >= options.cutOffNumber)) {
          best = {
            val: result,
            unit: possibility,
