...
var describe = function(resp) {
  ...
  , alternates: resp.unit.name.alternates || []
  };
}
...
/**
* Finds the unit based on abbreviation, singular, or plural
* within the optional system of measure
*/
Converter.prototype.lookup = function (query, measure) {
  var list = convert().list(measure);
  var queryLower = (query || '').toLowerCase();
  var foundUnit = false;
  each(list, function (unit) {
    if (unit.abbr.toLowerCase() == queryLower
      || unit.singular.toLowerCase() == queryLower
      || unit.plural.toLowerCase() == queryLower
    ) {
      foundUnit = unit;
      return false;
    }
    console.log('unit.alternates', unit.alternates);
    if (unit.alternates) {
      each(unit.alternates, function (alternateName) {
        if (alternateName.toLowerCase() == queryLower) {
          foundUnit = unit;
          return false;
        }
      });
